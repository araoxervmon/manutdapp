...
function searchUtil(item, toSearch) {
    /* Search Text in all 3 fields */
    return (item.homeTeam.toLowerCase().indexOf(toSearch.toLowerCase()) > -1 || item.awayTeam.toLowerCase().indexOf(toSearch.toLowerCase()) > -1 || item.goalsHomeTeam == toSearch || item.goalsAwayTeam == toSearch) ? true : false;
}
/*Get Player Data*/
function getPlayerData() {
	 var jqxhr = $.ajax({
		headers: { 'X-Auth-Token': 'f4e617dd900643c8a1a1c20585708274' },
		url: 'http://api.football-data.org/teams/66/fixtures/?season=2014',
		dataType: 'json',
		type: 'GET',
		async:false,
  }).done(function(data1) {
